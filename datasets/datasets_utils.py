...
def get_second_n_examples_id_each_class(Y_test, n=1):
    """
    Only return the classes with samples.
    """
    num_classes = Y_test.shape[1]
    ...
    for i in range(num_classes):
        loc = np.where(Y_test_labels == i)[0]
        if len(loc) > 0:
            selected_idx.append(list(loc[n:n + 1]))
...
def calculate_msqueezer_accuracy(Y_pred, Y_label):
    ...
    match_pred_vec = get_match_pred_vec(Y_pred, Y_label)
    for idx in range(Y_pred.shape[0]):  # successful attack reach concenses on majority
        # if np.max(Y_pred[idx]) < 0.49 and np.argmax(Y_pred[idx]) == np.argmax(Y_label[idx]):
        if np.sort(Y_pred[idx])[-1] == np.sort(Y_pred[idx])[-2] and np.argmax(Y_pred[idx]) == np.argmax(Y_label[idx]):
            alert_good = alert_good + 1
            # print (Y_pred[idx])

            '''
            #version 2
            if (np.argmax(Y_pred_list[0+idx])==np.argmax(Y_label[idx]) and np.max(Y_pred_list[0+idx])>np.max(Y_pred_list[Y_pred.shape[0]+idx]) and np.max(Y_pred_list[0+idx])>np.max(Y_pred_list[2*Y_pred.shape[0]+idx])) or (np.argmax(Y_pred_list[Y_pred.shape[0]+idx])==np.argmax(Y_label[idx]) and np.max(Y_pred_list[Y_pred.shape[0]+idx])>np.max(Y_pred_list[0+idx]) and np.max(Y_pred_list[Y_pred.shape[0]+idx])>np.max(Y_pred_list[2*Y_pred.shape[0]+idx])) or (np.argmax(Y_pred_list[2*Y_pred.shape[0]+idx])==np.argmax(Y_label[idx]) and np.max(Y_pred_list[2*Y_pred.shape[0]+idx])>np.max(Y_pred_list[0+idx]) and np.max(Y_pred_list[2*Y_pred.shape[0]+idx])>np.max(Y_pred_list[Y_pred.shape[0]+idx])):
                weight_count_good = weight_count_good+1
            '''

        # if np.max(Y_pred[idx]) < 0.49 and np.argmax(Y_pred[idx]) != np.argmax(Y_label[idx]):
        if np.sort(Y_pred[idx])[-1] == np.sort(Y_pred[idx])[-2] and np.argmax(Y_pred[idx]) != np.argmax(Y_label[idx]) or np.max(Y_pred[idx]) > 1:
        #if np.max(Y_pred[idx])>1:
            alert_bad = alert_bad + 1
        if alert_bad + np.sum(match_pred_vec) > len(Y_label):
            alert_bad = len(Y_label) - np.sum(match_pred_vec)

    accuracy = np.sum(match_pred_vec) / float(len(Y_label))
    # pdb.set_trace()

    # print (weight_count_good)
    return accuracy, alert_good, alert_bad
